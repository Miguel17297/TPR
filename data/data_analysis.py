...
## -- 2 -- ##
features_browsing = np.loadtxt("../cap/test.pcap_d0.1.dat")
features_yt = np.loadtxt("../cap/capture/dataset1.pcap_d0.1.dat")
...
features = np.vstack((features_yt, features_browsing))
oClass = np.vstack((oClass_yt, oClass_browsing))

print('Train Silence Features Size:', features.shape)
plt.figure(2)
plotFeatures(features, oClass, 0, 3)
# plt.figure(3)
...
i3Atest = np.vstack((testFeatures_browsing, testFeatures_yt))
...
nObsTest, nFea = i3Atest.shape
for i in range(nObsTest):
    print(' Kernel Linear->{:<10} | Kernel RBF->{:<10} | Kernel Poly->{:<10}'.format(AnomResults[L1[i]], AnomResults[L2[i]], AnomResults[L3[i]]))
